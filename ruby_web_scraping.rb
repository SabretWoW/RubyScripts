...
CSV.open(file,'w', 
  ...
  page.search("ol li.repo-leaderboard-list-item").each do |repo, i|
    # Create a temporary variable for each piece of repo data we want. The page to be scraped comes back in the form of a standard XML file and wrapped in a layer of Nokogiri goodness. Nokogiri is the Class that allows us to query & parse the page.
    rank = repo.search("a.leaderboard-list-rank").inner_text
